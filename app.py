...
def bump_usage(adzuna=0, jsearch=0, ai=0):
    month = datetime.now().strftime("%Y-%m")
    day   = datetime.now().strftime("%Y-%m-%d")
    with get_db() as conn:
        conn.execute(
            "INSERT INTO api_usage (month, adzuna_calls, jsearch_calls, ai_calls) VALUES (?,?,?,?) "
            "ON CONFLICT(month) DO UPDATE SET "
            "adzuna_calls=adzuna_calls+excluded.adzuna_calls, "
            "jsearch_calls=jsearch_calls+excluded.jsearch_calls, "
            "ai_calls=ai_calls+excluded.ai_calls, "
            "updated_at=datetime('now')",
            (month, adzuna, jsearch, ai)
        )
        if adzuna:
            conn.execute(
                "INSERT INTO api_usage_daily (day, adzuna_calls) VALUES (?,?) "
                "ON CONFLICT(day) DO UPDATE SET adzuna_calls=adzuna_calls+excluded.adzuna_calls",
                (day, adzuna)
            )
        conn.commit()
...
def get_usage():
    ...
    alimit = int(get_setting("adzuna_daily_limit", "250"))
    with get_db() as conn:
        mrow = conn.execute("SELECT * FROM api_usage WHERE month=?", (month,)).fetchone()
        drow = conn.execute("SELECT adzuna_calls FROM api_usage_daily WHERE day=?", (day,)).fetchone()
    j_used = mrow["jsearch_calls"] if mrow else 0
    az_month = mrow["adzuna_calls"] if mrow else 0
    az_today = drow["adzuna_calls"] if drow else 0
    ...
    return {
        ...
        "adzuna_today": az_today, "adzuna_daily_limit": alimit,
        "adzuna_remaining_today": max(0, alimit - az_today),
        "adzuna_pct": min(100, round(az_today/alimit*100)) if alimit else 0,
        "adzuna_month_total": az_month,
        "ai_calls": ai_calls,
    }
...
def trigger_scrape():
    ...
    adzuna_id  = get_setting("adzuna_app_id")
    adzuna_key = get_setting("adzuna_app_key")
    jsearch_key= get_setting("jsearch_key")
    purdue_key = get_setting("purdue_api_key")

    if not (adzuna_id and adzuna_key) and not jsearch_key:
        return jsonify({"ok": False, "msg": "No search API keys set. Add Adzuna (free) and/or JSearch in Settings."})
    if not purdue_key:
        return jsonify({"ok": False, "msg": "Purdue API key not set."})
    ...
    t = threading.Thread(target=run_scrape,
        args=(uid, user_dict, adzuna_id, adzuna_key, jsearch_key, purdue_key, locations, batch_id, skip_jsearch))
    t.daemon = True; t.start()
    return jsonify({"ok": True})

def run_scrape(uid, user, adzuna_id, adzuna_key, jsearch_key, purdue_key, locations, batch_id, skip_jsearch):
    started = datetime.now().isoformat()
    jobs_found = az_calls = js_calls = ai_calls = 0
    ...
    try:
        log("Fetching jobs from Adzuna + company boards...")
        jobs, az_calls, js_calls = scraper.scrape_jobs(
            adzuna_id, adzuna_key, jsearch_key, locations, log, skip_jsearch)
        ...
        bump_usage(adzuna=az_calls, jsearch=js_calls, ai=ai_calls)

        with get_db() as conn:
            conn.execute(
                "INSERT INTO scrape_log (user_id,started_at,finished_at,jobs_found,adzuna_calls,jsearch_calls,ai_calls,status) VALUES (?,?,?,?,?,?,?,?)",
                (uid,started,datetime.now().isoformat(),jobs_found,az_calls,js_calls,ai_calls,"success"))
            conn.commit()

        u = get_usage()
        log(f"✓ Done! {jobs_found} new jobs. Adzuna: {u['adzuna_today']}/{u['adzuna_daily_limit']} today | JSearch: {u['jsearch_used']}/{u['jsearch_limit']} this month")
        ...
        with get_db() as conn:
            conn.execute(
                "INSERT INTO scrape_log (user_id,started_at,finished_at,jobs_found,adzuna_calls,jsearch_calls,ai_calls,status) VALUES (?,?,?,?,?,?,?,?)",
                (uid,started,datetime.now().isoformat(),jobs_found,az_calls,js_calls,ai_calls,f"error: {e}"))
            conn.commit()
        log(f"ERROR: {e}")
    finally:
        scrape_status[uid]["running"] = False
...
def get_settings():
    ...
    return jsonify({
        ...
        "adzuna_app_id":        mask(get_setting("adzuna_app_id")),
        "adzuna_app_key":       mask(get_setting("adzuna_app_key")),
        ...
        "adzuna_daily_limit":   get_setting("adzuna_daily_limit") or "250",
...
def save_settings():
    data = request.json
    for k in ["purdue_api_key","jsearch_key","adzuna_app_id","adzuna_app_key",
              "purdue_api_model","purdue_api_url","jsearch_monthly_limit",
              "adzuna_daily_limit","sheets_id","sheets_auto_sync"]:
